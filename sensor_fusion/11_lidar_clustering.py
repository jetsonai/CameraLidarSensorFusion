...
left_image_paths = sorted(glob(os.path.join(DATA_PATH, 'image_03/data/*.png')))
...
##################################################################################
# LiDAR Clustering 추가 구현

#  LiDAR 포인트 클라우드를 카메라 이미지 좌표계로 변환하여 클러스터링된 포인트들을 반환
...
# 카메라 이미지 위에 클러스터링된 포인트들을 시각적으로 그려주
import matplotlib.cm as cm
...
cv2.imshow('Cluster LiDAR Projected Points', cv2.cvtColor(new_image, cv2.COLOR_RGB2BGR))  # RGB에서 BGR로 변환 필요
cv2.waitKey(0)
