    ...
    if min_points_in_cluster <= len(cluster_indices) <= max_points_in_cluster:
        ...
        if min_z_value <= z_min and z_max <= max_z_value:
            ...
            if min_height <= height_diff <= max_height:
                ...
                bbox.color = (1, 0, 0)  # 바운딩 박스 색상 
                bboxes_123.append(bbox)

# 시각화: 1번, 2번, 3번 조건을 모두 만족하는 클러스터와 바운딩 박스
o3d.visualization.draw_geometries([final_point] + bboxes_123, 
                                  window_name="Clusters filtered by Point Count, Z Range, and Height Difference (Condition 1, 2 & 3)")
