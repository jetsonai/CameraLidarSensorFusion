...
import torch  # 추가하여 CUDA 사용 가능성 체크

# YOLOv8 설정 (CUDA를 사용할 수 있도록 설정)
model = YOLO('/home/user/sensor_fusion/camera_only/yolov8n.pt').to('cuda')

# GPU에서 사용할 수 있는지 확인
if not torch.cuda.is_available():
    print("CUDA is not available. YOLO inference will run on CPU instead.")
    model = model.to('cpu')
...
def get_2d_bboxes(camera_box_points):
    ...
    for box_pts in camera_box_points:
        ...
        # Ensure the output is in the form [[x_min, y_min], [x_max, y_max]]
...
def calculate_iou(box1, box2):
    ...
    if isinstance(box1[0], (list, tuple)) and isinstance(box2[0], (list, tuple)):
        x1, y1 = max(box1[0][0], box2[0][0]), max(box1[0][1], box2[0][1])
        x2, y2 = min(box1[1][0], box2[1][0]), min(box1[1][1], box2[1][1])
        ...
        raise ValueError("Input boxes must be in the format [[x_min, y_min], [x_max, y_max]]")
...
# edit not using CPU, only using GPU
def main_pipeline(left_image, lidar_bin, velo_points):
    # YOLO를 통해 2D 바운딩 박스 추출 (빨간색)
    yolo_detections = model(left_image)
    yolo_bboxes = yolo_detections[0].boxes.xyxy.cpu().numpy()  # Convert to numpy for list conversion
