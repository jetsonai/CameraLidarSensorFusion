...
# YOLOv8 설정
model = YOLO('../../camera_only/yolov8n.pt')

DATA_PATH = r'../../datasets/drive_0106_sync/'

left_image_paths = sorted(glob(os.path.join(DATA_PATH, 'image_02/data/*.png')))
bin_paths = sorted(glob(os.path.join(DATA_PATH, 'velodyne_points/data/*.bin')))

# 카메라 보정 데이터 가져오기
with open('../../datasets/calib_cam_to_cam.txt', 'r') as f:
    calib = f.readlines()
...
with open(r'../../datasets/calib_velo_to_cam.txt', 'r') as f:
    calib = f.readlines()
...
#output_video = cv2.VideoWriter('../../sensor_fusion/test_video.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 10, (frame_width, frame_height))
...
for index in range(min(len(left_image_paths), 60)):
    ...
    #cv2.imshow('sensor_fusion_image', cv2.cvtColor(sensor_fusion_image, cv2.COLOR_RGB2BGR))
