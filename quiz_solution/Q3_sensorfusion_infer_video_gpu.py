...
# YOLOv8 설정
model = YOLO('/home/user/sensor_fusion/camera_only/yolov8n.pt').to('cuda')  # GPU 사용

DATA_PATH = r'../../dataset/2011_09_26/2011_09_26_drive_0106_sync'
left_image_paths = sorted(glob(os.path.join(DATA_PATH, 'image_02/data/*.png')))
bin_paths = sorted(glob(os.path.join(DATA_PATH, 'velodyne_points/data/*.bin')))

# 카메라 보정 데이터 가져오기
with open('../../dataset/2011_09_26/calib_cam_to_cam.txt', 'r') as f:
    calib = f.readlines()
...
with open(r'../../dataset/2011_09_26/calib_velo_to_cam.txt', 'r') as f:
    calib = f.readlines()
...
def main_pipeline(left_image, lidar_bin, velo_points):
    ...
    yolo_bboxes = yolo_detections[0].boxes.xyxy  # GPU 텐서 그대로 사용
    ...
    return sensor_fusion_bbox

# 동영상 설정 (6fps)
frame_width, frame_height = cv2.imread(left_image_paths[0]).shape[1], cv2.imread(left_image_paths[0]).shape[0]
output_video = cv2.VideoWriter('sensor_fusion.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 10, (frame_width, frame_height))

# 모든 프레임 처리 (최대 60 프레임)
for index in range(min(len(left_image_paths), 60)):
    left_image = cv2.cvtColor(cv2.imread(left_image_paths[index]), cv2.COLOR_BGR2RGB)
    lidar_bin = bin_paths[index]
    velo_points = bin2h_velo(lidar_bin, remove_plane=True)

    sensor_fusion_image = main_pipeline(left_image, lidar_bin, velo_points)

    # 동영상에 프레임 추가
    output_video.write(cv2.cvtColor(sensor_fusion_image, cv2.COLOR_RGB2BGR))
    print("frame num : ", index)
